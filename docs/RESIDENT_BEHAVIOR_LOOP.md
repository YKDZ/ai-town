...
┌─────────────────────────────────────────────────────────────────┐
│                         主模拟循环                                │
│  （位于 src/core/simulation.py 的 update() 方法）                │
└─────────────────────────────────────────────────────────────────┘
                                ↓
                    ┌───────────────────────┐
                    │   1. 时间推进          │
...
### 1. 时间推进
- 每个循环周期推进游戏时间（默认每次 tick 推进5分钟，可通过 `Config.MINUTES_PER_TICK` 配置）
- 维护昼夜交替机制
- 跟踪当前日期、星期和具体时间
...
系统的核心行为参数在 `src/config.py` 中定义，主要包括：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `SIMULATION_DURATION_DAYS` | 3 | 模拟总时长（天） |
| `MINUTES_PER_TICK` | 5 | 每个循环周期推进的游戏时间（分钟） |
| `INTERACTION_COOLDOWN_MINUTES` | 150 | 两人对话后的冷却时间（分钟） |
| `INTERACTION_PROBABILITY` | 0.4 | 满足条件时发生交互的概率（0.0-1.0） |
| `CONVERSATION_BUSY_DURATION` | 45 | 对话后保持忙碌状态的时长（分钟） |
| `DEFAULT_ACTION_DURATION` | 60 | 行为的默认持续时长（分钟） |
| `PLANNING_RETRY_DELAY` | 15 | 规划失败时的重试延迟（分钟） |
